 ...
 * $cantidad = urlencode($cantidad); $moneda_origen = urlencode($moneda_origen);
 * $moneda_destino = urlencode($moneda_destino); $url =
 * "http://www.google.com/ig/calculator?hl=en&q=$cantidad$moneda_origen=?$moneda_destino";
 * $ch = curl_init(); $timeout = 0; curl_setopt ($ch, CURLOPT_URL, $url);
 * curl_setopt ($ch, CURLOPT_RETURNTRANSFER, 1); curl_setopt($ch,
 * CURLOPT_USERAGENT , "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1)");
 * curl_setopt ($ch, CURLOPT_CONNECTTIMEOUT, $timeout); $rawdata =
 * curl_exec($ch); curl_close($ch); $data = explode('"', $rawdata); $data =
 * explode(' ', $data['3']); $var = $data['0']; return round($var,3); }
 */
...
function updateCacheCurrency() {
	...
	if (($json_array ["updated"] != $currentDate) && ! isset ( $_SERVER ["REMOTE_ADDR"] )) { // refreshCache
		...
		@mail ( "jose.nobile@latinoaustralia.com", "Actualizando currency " . date ( "Y-m-d H:i:s" ), $body, "From:database@cali.latinoaustralia.com" );
		foreach ( $exchanges as $exchange ) {
			foreach ( $exchanges as $exchange2 ) {
				if ($exchange2 == $exchange) {
					...
					if ($result === false) {
						// echo "Invalid convertion from $exchange to $exchange2
						// skiping <br />\r\n";
						continue;
					}
		...
		// $json_array["olds"][$currentDate] = $newArray;//uncomment to save the
		// historial
		...
		@mail ( "jose.nobile@latinoaustralia.com", "Actualizada currency " . date ( "Y-m-d H:i:s" ), $body, "From:database@cali.latinoaustralia.com" );
	}
...
function getMime($filename) {
	preg_match ( "/\.(.*?)$/", $filename, $m ); // Get File extension for a better
	                                         // match
	$m [1] = str_replace ( ".", "", $m [1] );
	$m [1] = trim ( $m [1] );
	// print_r($m);
	...
	switch (strtolower ( $m [1] )) {
		...
		default :
			if (function_exists ( "mime_content_type" )) { // if mime_content_type
			                                          // exists use it.
				...
				if (strstr ( $_SERVER [HTTP_USER_AGENT], "Windows" )) { // Nothing to do
				                                                  // on windows
					return ""; // Blank mime display most files correctly especially
					           // images.
				}
				if (strstr ( $_SERVER [HTTP_USER_AGENT], "Macintosh" )) { // Correct
				                                                    // output on macs
...
function seoUrl($url) {
	...
	for($i = 0; $i < strlen ( $url ); $i ++) {
		$rchr = $url {$i};
		$chr = ord ( $rchr ); // ASCII
		if ((! ($chr <= 47 || ($chr >= 58 && $chr <= 64) || ($chr >= 91 && $chr <= 96) || $chr >= 123)) || $chr == 32 || $rchr == '-' || $rchr == '/') { // Escrito
		                                                               // a mano
		                                                               // limpia
			$nurl .= $rchr;
		} // if
...
/*
 * function money_format($format, $number) { $regex =
 * '/%((?:[\^!\-]|\+|\(|\=.)*)([0-9]+)?'.
 * '(?:#([0-9]+))?(?:\.([0-9]+))?([in%])/'; if (setlocale(LC_MONETARY, 0) ==
 * 'C') { setlocale(LC_MONETARY, ''); } $locale = localeconv();
 * preg_match_all($regex, $format, $matches, PREG_SET_ORDER); foreach ($matches
 * as $fmatch) { $value = floatval($number); $flags = array( 'fillchar' =>
 * preg_match('/\=(.)/', $fmatch[1], $match) ? $match[1] : ' ', 'nogroup' =>
 * preg_match('/\^/', $fmatch[1]) > 0, 'usesignal' => preg_match('/\+|\(/',
 * $fmatch[1], $match) ? $match[0] : '+', 'nosimbol' => preg_match('/\!/',
 * $fmatch[1]) > 0, 'isleft' => preg_match('/\-/', $fmatch[1]) > 0 ); $width =
 * trim($fmatch[2]) ? (int)$fmatch[2] : 0; $left = trim($fmatch[3]) ?
 * (int)$fmatch[3] : 0; $right = trim($fmatch[4]) ? (int)$fmatch[4] :
 * $locale['int_frac_digits']; $conversion = $fmatch[5]; $positive = true; if
 * ($value < 0) { $positive = false; $value *= -1; } $letter = $positive ? 'p' :
 * 'n'; $prefix = $suffix = $cprefix = $csuffix = $signal = ''; $signal =
 * $positive ? $locale['positive_sign'] : $locale['negative_sign']; switch
 * (true) { case $locale["{$letter}_sign_posn"] == 1 && $flags['usesignal'] ==
 * '+': $prefix = $signal; break; case $locale["{$letter}_sign_posn"] == 2 &&
 * $flags['usesignal'] == '+': $suffix = $signal; break; case
 * $locale["{$letter}_sign_posn"] == 3 && $flags['usesignal'] == '+': $cprefix =
 * $signal; break; case $locale["{$letter}_sign_posn"] == 4 &&
 * $flags['usesignal'] == '+': $csuffix = $signal; break; case
 * $flags['usesignal'] == '(': case $locale["{$letter}_sign_posn"] == 0: $prefix
 * = '('; $suffix = ')'; break; } if (!$flags['nosimbol']) { $currency =
 * $cprefix . ($conversion == 'i' ? $locale['int_curr_symbol'] :
 * $locale['currency_symbol']) . $csuffix; } else { $currency = ''; } $space =
 * $locale["{$letter}_sep_by_space"] ? ' ' : ''; $value = number_format($value,
 * $right, $locale['mon_decimal_point'], $flags['nogroup'] ? '' :
 * $locale['mon_thousands_sep']); $value =
 * @explode($locale['mon_decimal_point'], $value); $n = strlen($prefix) +
 * strlen($currency) + strlen($value[0]); if ($left > 0 && $left > $n) {
 * $value[0] = str_repeat($flags['fillchar'], $left - $n) . $value[0]; } $value
 * = implode($locale['mon_decimal_point'], $value); if
 * ($locale["{$letter}_cs_precedes"]) { $value = $prefix . $currency . $space .
 * $value . $suffix; } else { $value = $prefix . $value . $space . $currency .
 * $suffix; } if ($width > 0) { $value = str_pad($value, $width,
 * $flags['fillchar'], $flags['isleft'] ? STR_PAD_RIGHT : STR_PAD_LEFT); }
 * $format = str_replace($fmatch[0], $value, $format); } return $format; }
 */
function showMoney($float, $currencyIn = "AUD", $currencyTo = '') {
	if ($currencyTo == '')
		$currencyTo = $currencyIn;
		/*
	 * if(function_exists("money_format")===TRUE){ $money = money_format('%i',
	 * $float); }
	 */
