...
$t->is ( array_keys ( $engine->getRenderers () ), array (
		'php' 
...
$t->ok ( $engine->getRenderers () === array (
		'php' => $renderer 
...
$engine = new ProjectTemplateEngine ( $loader );
$engine->setHelperSet ( new sfTemplateHelperSet ( array (
		'foo' => $helper = new SimpleHelper ( 'bar' ) 
) ) );
...
$t->is ( $engine->render ( 'foo', array (
		'foo' => 'foo' 
) ), '-barfoo-', '->render() uses the decorator to decorate the template' );
...
$engine = new ProjectTemplateEngine ( new CompilableTemplateLoader (), array (
		'foo' => new FooTemplateRenderer () 
) );
