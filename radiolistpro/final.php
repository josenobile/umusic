...
$lGeneros[$archivitos['GENERO']][] = $archivitos;
}
//print_r($lGeneros);
...
$posibleArtista = $posibleMP3['ARTISTA'];
				...
				$sePuede = TRUE;
					...
					flush();
						if($cancion['TITULO'] == $posibleTitulo){
						$sePuede = FALSE;
				...
				//Verificar que no se halla puesto una cancion del artista en la lista
					...
					//echo ".";
					//@ob_flush();
						if(strlen($cancion['ARTISTA'])> 0 
						&& strlen($posibleArtista) > 0 
						&& @stristr($cancion['ARTISTA'],$posibleArtista)!== FALSE 
						|| @stristr($posibleArtista,$cancion['ARTISTA'])!==FALSE){
...
$Result1 = mysql_query($insertSQL, $radiocomunicate) or die(mysql_error());
					...
					//
					$j++;
					echo "<tr><td>".$j."</td><td>".$posibleMP3['GENERO']."</td><td>".$posibleMP3['ARTISTA']."</td><td>" .$posibleMP3['TITULO']."</td><td>".
