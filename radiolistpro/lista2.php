...
set_time_limit(0);
ini_set("memory_limit","-1");require_once('../../Connections/radiocomunicate.php');
//Inicio Xajax
...
$xajax->registerFunction("cargarArtistasPorGenero"); 
function cargarArtistasPorGenero($genero,$id)
{

global $database_radiocomunicate, $radiocomunicate;

mysql_select_db($database_radiocomunicate, $radiocomunicate);
$query_artistas = sprintf("SELECT artista as artistatag, path FROM hacerlist_audiofiles WHERE genero = %s GROUP BY path", GetSQLValueString($genero, "text"));
$artistas = mysql_query($query_artistas, $radiocomunicate) or die(mysql_error());
...
?>
<?PHP
	$path = $row_artistas['path'];
	$ppath = explode("/",$path);
	if($ppath[4]==$artista)continue;$artista = $ppath[4];
?>
  <option value="<?php echo $ppath[3].'/'.$artista;?>"><?php echo $artista;?></option>
  <?php
...
$objResponse->addAssign($id,"innerHTML", $imagen);
...
?>
<form action="final2.php" method="post" enctype="application/x-www-form-urlencoded" target="_blank" name="hacerLista" id="hacerLista">
<?PHP /* ?>
...
<div class="generos">
<select name="generos[]" title="0" onChange="xajax_cargarArtistasPorGenero(this.value,'artistas'+this.title);">
...
<span id="artistas0">Seleccione</span>
<input type="button" value="+" />  - <input type="button" value="-" />
</div>
<br />
<?PHP
...
?>
<br />
Nombre de la lista: <input type="text" name="nombreLista" value="<?PHP echo time();?>" /><br />
Opciones:
	...
	$(function() {
		$(".separadores").each(function () {
			$(this).children().eq(1).click(function () {
				$(this).parent().after($(this).parent().clone(true)).hide().show("fast");
			});
		...
		$(".generos").each(function () {
			$(this).children().eq(2).click(function () {
				window.laid++;
				var obj = $(this).parent().after($(this).parent().clone(true)).hide().show("fast").children();
				obj.eq(1).attr("id",'artistas'+window.laid);
				obj.eq(0).attr("title",window.laid);
			});
